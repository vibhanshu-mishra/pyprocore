...
- Previous stable release: `2.3.0`
- `v2.4.0` is published on PyPI and released on GitHub.
- `2.3.0` included the additive work completed after `2.2.0`: Phase 8 expanded
...
- The published `2.4.0` release includes Phase 16A
  ...
  The package was published to PyPI and tagged and released on GitHub.
...
risk summaries, and optional output-directory artifacts. Phase 18E adds local
API compatibility contracts, validation, diffs, and codebase compatibility
reports.
...
Phase 17C is template/report oriented only. It does not host infrastructure,
...
Phase 18B scans user-selected local folders using AST and lexical inspection.
It does not clone or fetch repositories, execute or import scanned files, edit
customer code, generate patches, commits, branches, or pull requests, call
...
Phase 18E contracts describe supported families, local-only features,
deprecations, known gaps, and safety boundaries. They are local metadata and
