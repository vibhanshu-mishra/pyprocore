...
Set PROCORE_PROJECT_ID or PROCORE_PROJECT_NAME, and set PROCORE_SUBMITTAL_ID or
PROCORE_SUBMITTAL_NUMBER. This script makes live Procore API calls and may
download attachments unless PROCORE_NO_DOWNLOADS is set to "1".
"""
...
def main() -> None:
    """Run the submittal workflow package example."""
    ...
    try:
        ...
        print(f"Could not build the submittal package: {error}")
        return

    print("Built submittal workflow package:")
    print(f"- Project ID: {package.project_id}")
    print(f"- Submittal ID: {package.item_id}")
