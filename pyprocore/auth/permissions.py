...
def explain_auth_error(status_code: int, response_body: Any = None, auth_mode: Any = None) -> str:
    ...
    if status_code == 401:
        renewal = (
            "PyProcore requests a new access token; client-credentials tokens do not need a refresh token."
            if mode is AuthMode.CLIENT_CREDENTIALS
            else "Reauthorize if the access token is expired and no usable refresh token is stored."
        )
        explanation = f"401 Unauthorized usually means a missing, expired, malformed, or wrong-environment token. {renewal}"
    elif status_code == 403:
        explanation = explain_permission_error(status_code, response_body, mode)
    else:
        explanation = f"HTTP {status_code} is not a standard OAuth authentication status; inspect the safe server message and request context."
...
def explain_permission_error(
    ...
    return (
        ...
        "or the app is not connected to the company. Confirm the app-company connection and Procore permissions."
    )
...
def explain_app_connection_issue(auth_mode: Any = None) -> str:
    ...
    mode = normalize_auth_mode(auth_mode)
    if mode is AuthMode.CLIENT_CREDENTIALS:
        return "Client Credentials access requires the Data Connection App to be installed/connected for the target company and its service account to have the required permissions."
    return "Authorization Code access depends on the installed app configuration and the authenticated user's company, project, and tool permissions."
...
def explain_environment_mismatch(login_url: str | None, api_base: str | None) -> str:
    ...
    values = f"{login_url or ''} {api_base or ''}".casefold()
    ...
    if appears_mixed:
        return "The configured login and API URLs appear to target different environments. Sandbox credentials and URLs must stay together, as must production credentials and URLs."
    return "Confirm that the credentials, login URL, API URL, and target company all belong to the same Procore environment (sandbox or production)."
