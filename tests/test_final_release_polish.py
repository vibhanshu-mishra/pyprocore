...
class FinalReleasePolishTestCase(unittest.TestCase):
    ...
    def test_docs_claim_2_4_0_release_is_complete(self) -> None:
        """Release docs should describe the completed 2.4.0 release."""
        checked_docs = "\n".join(
            self.read_text(path)
            for path in (
                "docs/project-status.md",
        ...
        self.assertIn("PyProcore `2.4.0` has been published to PyPI", checked_docs)
        self.assertIn("released on GitHub", checked_docs)
        self.assertNotIn("2.4.0 has not been published", checked_docs)
