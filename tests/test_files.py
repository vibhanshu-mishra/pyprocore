    ...
    def test_download_url_streams_file_with_bearer_token(self) -> None:
        ...
        with TemporaryDirectory() as temporary_directory:
            ...
            saved_path = self.service.download_url(
                "https://signed.example/file.txt", destination
            )
        ...
        self.assertEqual(
            request_kwargs["headers"]["Authorization"], "Bearer access-token"
        )
    ...
    def test_module_level_download_url_uses_file_service(self) -> None:
        ...
        with TemporaryDirectory() as temporary_directory:
            ...
            with unittest.mock.patch(
                "services.files.requests.Session"
            ) as session_class:
                session = Mock()
                ...
                self.assertEqual(
                    download_url(
                        "https://signed.example/file.txt", destination, token_manager
                    ),
                    destination,
                )
