...
class DocsTruthAuditTestCase(unittest.TestCase):
    ...
    def test_roadmap_places_future_items_under_future(self) -> None:
        ...
        for phrase in ("Observations", "Punch item", "correspondence", "client credentials"):
            self.assertIn(phrase.lower(), released_section.lower())
        self.assertIn("Phase 12", released_section)
        self.assertIn("AI workflow examples", released_section)
        self.assertIn("Phase 10A-10E async client", released_section)
        self.assertIn("Phase 11A-11D metadata-only plugin", released_section)
        self.assertIn("Phase 13A-13D local deterministic", released_section)
        self.assertIn("Phase 15A-15C discovery-only MCP", released_section)
    ...
    def test_examples_readme_documents_examples_as_released(self) -> None:
        """Examples README should describe current example ranges."""
        ...
        self.assertIn("Examples `305` through `309`", examples)
        self.assertIn("Examples `310` through `314`", examples)
