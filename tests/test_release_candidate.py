...
class ReleaseCandidateTestCase(unittest.TestCase):
    ...
    def test_project_status_mentions_completed_2_4_0_release(self) -> None:
        """Project status page should reflect the completed 2.4.0 release."""
        report = self.read_text("docs/project-status.md")

        self.assertIn("Current stable release: `2.4.0`", report)
        self.assertIn("Previous stable release: `2.3.0`", report)
        self.assertIn("`v2.4.0` is published on PyPI and released on GitHub.", report)
